...
function getAverageMark(student) {
    const allMark = Object.values(student.subjects).flat(1);
    return (allMark.reduce((a, b) => a + b) / allMark.length).toFixed(2);
}
  
console.log(`Повертає середню оцінку по усім предметам для переданого студента`, getAverageMark(students[2]));
...
console.log(`Повертає інформацію загального виду по переданому студенту`, getStudentInfo(students[1]));
...
console.log(`Повертає кращого студента зі списку по показнику середньої оцінки`, getBestStudent(students));
...
function calculateWordLetters(string) {
    ...
    const result = arrString.reduce((letter, current) => {
...
console.log(`Повертає обє'кт, в якому ключі це букви у слові, а значення – кількість їх повторень`, calculateWordLetters('веделка'));